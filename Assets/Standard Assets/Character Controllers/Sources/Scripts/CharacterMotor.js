...
class CharacterMotorJumping {
	...
	// How many times can the character jump (when not grounded) in a row?
	// 0 is normal jumping
	var airJumps: int = 0;

	// How high do we jump when pressing jump and letting go immediately, in the air?
	var airJumpHeight : float = 1.0;
	
	// We add extraHeight units (meters) on top when holding the button down longer while jumping
	var airJumpExtraHeight : float = 4.1;
...
private function ApplyGravityAndJumping (velocity : Vector3) {
	...
	if (grounded)
		...
		if (jumping.jumping && jumping.holdingJumpButton) {
			...
			if(jumping.airJumpsMade > 0) {
				if (Time.time < jumping.lastStartTime + jumping.airJumpExtraHeight / CalculateJumpVerticalSpeed(jumping.airJumpHeight)) {
	...
	if (grounded) {
		...
		if (jumping.enabled && canControl && (Time.time - jumping.lastButtonDownTime < 0.2)) {
			velocity = Jump(velocity, false);
		}
		else {
			jumping.holdingJumpButton = false;
		}
	} else if (jumping.airJumpsMade < jumping.airJumps) {
		if (jumping.enabled && canControl && (Time.time == jumping.lastButtonDownTime) && !jumping.holdingJumpButton) {
			velocity = Jump(velocity, true);
		}
	}
...
private function Jump(velocity : Vector3, isAirJump : boolean) {
	grounded = false;
	...
	if(isAirJump) jumping.airJumpsMade++;
